...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--use-ai", action="store_true", help="Use OpenAI API instead of deterministic fallback output")
...
def main() -> int:
    ...
    config = AppConfig.from_root(args.root, max_files_per_run=args.max_files, use_ai=args.use_ai, model=args.model)
    outputs = run(config)
    if not outputs:
