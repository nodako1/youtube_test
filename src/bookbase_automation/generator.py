...
def _scene_06_structured_prompt(context: dict[str, object]) -> dict[str, object]:
    ...
    deep_fields = _deep_dive_fields(scene6_body, point_1_label, "重要ポイント①", 6)
    visual_structure = str(deep_fields["visual_structure"])
    ...
    composition = _deep_dive_composition(visual_structure, 6)
    motifs = ["理由カード", "仕組みカード", "結果カード", "細い接続線", "半透明レイヤー"]
    final_prompt = f"""Create a 16:9 landscape video-insert image for Book Base, a Japanese business book YouTube channel. Use a refined watercolor illustration style with a premium, calm, elegant atmosphere. Use a soft cream-white and beige background with teal and subtle gold accents. Leave the lower-left corner clean because the fixed Book Base logo will be composited after generation.

This is Scene 06. Its fixed role is to deeply explain Key Point 1 from Scene 05 by turning the script into a strong claim, a misconception-versus-correct-view diagram, framework elements, a concrete scene illustration, and a bottom takeaway ribbon. Do not create a generic business person image. Do not hard-code any topic from a previous book. Build the visual message from the current script only.
...
Keep the image clean and easy to understand at a glance. Use readable short Japanese text, including a bottom takeaway ribbon. Do not place long script text. Avoid clutter, avoid generic emotional icons, avoid a generic desk-only scene, avoid over-explaining, and avoid repeating the Scene 05 composition."""
    return {"scene": 6, "fixed_role": "重要ポイント①の深掘り", "scene_role": "重要ポイント①の深掘り", "point_1_label": point_1_label, "core_message": core_message, "scene_06_core_message": core_message, "reason_label": elements[0], "mechanism_label": elements[1], "effect_label": elements[2], "visual_metaphor": visual_metaphor, "visual_structure": visual_structure, "exact_text_elements": elements, "composition": composition, "visual_motifs": motifs, "style": _COMMON_STYLE_FOR_SCHEMA, "negative_rules": ["特定の本の語句を固定しない", "過去テーマの語句を使い回さない", "generic emotional iconsを使わない", "ただのビジネス人物イラストにしない", "scene_05と同じ構図にしない", "文字を増やしすぎない"], "variation_key": f"scene-06-{visual_structure}-key-point-1-mechanism", "final_prompt": final_prompt, **deep_fields}
...
def _scene_10_structured_prompt(context: dict[str, object]) -> dict[str, object]:
    ...
    deep_fields = _deep_dive_fields(scene10_body, point_2_label, "重要ポイント②", 10)
    visual_structure = str(deep_fields["visual_structure"])
    ...
    composition = _deep_dive_composition(visual_structure, 10)
    visual_motifs = ["原稿由来の具体例", "実践カード", "短いキーワード", "Scene 09と異なる構図", "余白"]
    final_prompt = f"""Create a 16:9 landscape video-insert image for Book Base, a Japanese business book YouTube channel. Use a refined watercolor illustration style with a premium, calm, elegant atmosphere. Use a soft cream-white and beige background with teal and subtle gold accents. Leave the lower-left corner clean because the fixed Book Base logo will be composited after generation.

This is Scene 10. Its fixed role is to deeply explain Key Point 2 from Scene 09 by turning the script into a strong claim, a misconception-versus-correct-view diagram, framework elements, a concrete scene illustration, and a bottom takeaway ribbon. Do not create a generic cause-and-effect flowchart unless the current script actually requires it. Do not hard-code any topic from a previous book. Build the visual message from the current script only.
...
Keep the image clean and easy to understand at a glance. Use readable short Japanese text, including a bottom takeaway ribbon. Do not place long script text. Avoid clutter, avoid generic split-screen flowcharts, avoid generic business people, avoid English text, and avoid repeating the Scene 09 composition."""
    return {"scene": 10, "fixed_role": "重要ポイント②の深掘り", "scene_role": "重要ポイント②の深掘り", "core_message": scene_10_core_message, "point_2_label": point_2_label, "scene_10_core_message": scene_10_core_message, "example_label": example_label, "application_label": application_label, "result_label": result_label, "visual_structure": visual_structure, "visual_metaphor": visual_metaphor, "exact_text_elements": elements, "composition": composition, "visual_motifs": visual_motifs, "style": _COMMON_STYLE_FOR_SCHEMA, "negative_rules": ["split-screenを固定しない", "cause-and-effectを固定しない", "ビジネス会話シーンを固定しない", "scene_09と同じ構図にしない", "scene_11の実話エピソード風にしない", "長文や英語テキストを入れない"], "variation_key": f"scene-10-key-point-2-deep-dive-{visual_structure}", "final_prompt": final_prompt, **deep_fields}
...
def _scene_14_structured_prompt(context: dict[str, object]) -> dict[str, object]:
    ...
    deep_fields = _deep_dive_fields(scene14_body, point_3_label, "重要ポイント③", 14)
    visual_structure = str(deep_fields["visual_structure"])
    ...
    composition = _deep_dive_composition(visual_structure, 14)
    visual_motifs = ["原稿由来の具体場面", "3つ以内の短い日本語ラベル", "小さな実践モチーフ", "scene_13とは違う構図", "scene_18とは違う一例への絞り込み"]
    final_prompt = f"""Create a 16:9 landscape video-insert image for Book Base, a Japanese business book YouTube channel. Use a refined watercolor illustration style with a premium, calm, elegant atmosphere. Use a soft cream-white and beige background with teal and subtle gold accents. Leave the lower-left corner clean because the fixed Book Base logo will be composited after generation.

This is Scene 14. Its fixed role is to deeply explain Key Point 3 from Scene 13 by turning the script into a strong claim, a misconception-versus-correct-view diagram, framework elements, a concrete scene illustration, and a bottom takeaway ribbon. Do not create a generic meeting scene unless the current script actually requires it. Do not hard-code any topic from a previous book. Build the visual message from the current script only.
...
scene_14の固定役割は「重要ポイント③の深掘り」です。scene_14ではscene_13の重要ポイント③とscene_14本文から、scene_number, point_number, main_concept, misconception, correct_view, framework_elements, process_flow, before_after, visual_metaphor, takeaway_label, point_3_label, scene_14_core_message, example_context_label, action_label, result_label, visual_structure, visual_metaphorを毎回抽出してください。visual_structureはpractical_example / step_demo / before_after / tool_use / scenario_demo / framework_applicationから現在の内容に合わせて1つ選び、構図に反映してください。exact_text_elementsは3つ以内、原稿由来の短い日本語のみとし、英語や長文を入れないでください。meeting、businessperson calmly responding、会議、返答、上司への対応、コミュニケーション、言い換え、具体的フレーズを恒久テンプレートに固定しないでください。scene_14はscene_13の導入カードを繰り返さず、scene_18の全体実践シーンとも役割を分けてください。
