...
_COMMON_STYLE_FOR_SCHEMA = "16:9 landscape, watercolor illustration, premium calm atmosphere, Japanese business book YouTube channel, cream white, beige, teal, subtle gold palette, clean but information-rich composition, enough whitespace, fixed Book Base logo will be composited in post-processing, concise readable Japanese text, do not place long script text, vary composition from adjacent scenes"
...
SCENE_04_COMPOSITION = "16:9固定。左側40%に著者の大きな水彩イラストまたは上品なシルエットを置き、右上に自然な見出し『著者 〜さん』と短い肩書き、右中に①②③の順で重要ポイントカードを縦のステップ状に配置する。カード同士をティールとゴールドの細い矢印や線でつなぎ、右下または下部に『理解から実践へ』のような補足帯を入れて、3つを順番に学ぶ意味を見せる。淡いクリーム背景、ティール・ゴールドのアクセント、高級感・知性・やさしさを保ち、Book Baseロゴは描かず左下を後処理用に空ける。"
...
SCENE_04_FORBIDDEN_META_PHRASES = [
    "【シーン4】ずっと【シーン4】のままにしてください",
    "省略しないでください",
    "改行は本文内に入れないでください",
    "【シーン4】のままにしてください",
    "【シーン4】のままにします",
...
def _remove_scene4_meta_instructions(text: str) -> str:
    cleaned = " ".join(text.splitlines())
    for phrase in SCENE_04_FORBIDDEN_META_PHRASES:
        cleaned = cleaned.replace(phrase, "")
    cleaned = re.sub(r"\s+", " ", cleaned).strip()
    return cleaned


def _extract_scene4_key_points(scene4: str) -> list[str]:
    match = re.search(r"本書の重要ポイントは、(.+?)です。", scene4)
    if match:
        return [part.strip(" 、。") for part in re.split(r"、", match.group(1)) if part.strip(" 、。")][:3]
    return [m.strip(" 、。") for m in re.findall(r"[①②③]\s*(.*?)(?:、|。|です|$)", scene4) if m.strip(" 、。")][:3]
...
def build_image_context(script: str, book_title: str, asset_checks: list[AssetCheck] | None = None) -> dict[str, object]:
    ...
    point_matches = _extract_scene4_key_points(scene4)
    labels = [_short_label(m, f"ポイント{i}", 24) for i, m in enumerate(point_matches[:3], 1)]
    while len(labels) < 3:
    ...
    context = {
        ...
        "three_key_points": [
            ...
            {"index": 2, "label": labels[1], "core_message": _short_label(scene9, labels[1], 30)},
            {"index": 3, "label": labels[2], "core_message": _short_label(scene13, labels[2], 30)},
        ],
        "scene_bodies": {"scene_02": scene2, "scene_03": scene3, "scene_04": scene4, "scene_05": scene5, "scene_06": scene6, "scene_07": scene7, "scene_09": scene9, "scene_10": scene10, "scene_11": scene11, "scene_12": scene12, "scene_13": scene13, "scene_14": scene14, "scene_15": scene15, "scene_16": scene16, "scene_17": scene17, "scene_18": scene18, "scene_20": scene20},
        "assets": {
...
def _scene_03_structured_prompt(context: dict[str, object]) -> dict[str, object]:
    ...
    scene_bodies = context.get("scene_bodies", {})
    scene2_body = str(scene_bodies.get("scene_02", "")) if isinstance(scene_bodies, dict) else ""
    scene3_body = str(scene_bodies.get("scene_03", "")) if isinstance(scene_bodies, dict) else ""
    conclusion_match = re.search(r"結論から言うと、(.+?)です。", scene3_body) or re.search(r"そこで今回は、(.+?)について学んでいきます。", scene2_body)
    conclusion = _short_label(conclusion_match.group(1) if conclusion_match else str(context.get("current_theme", "")), "本の価値が仕事に活きる", 18)
    support = _short_label(str(context["three_key_points"][0]["core_message"]), "実践の視点がわかる", 14)
    key_points = [str(item["label"]) for item in context["three_key_points"][:3]]
    target = _short_label(scene3_body or scene2_body, "仕事に活かしたい人", 14)
    elements = ["今回の1冊", conclusion, support, *key_points[:3], f"{target}におすすめ"]
    final_prompt = f"""Create a 16:9 landscape video-insert image for Book Base, a Japanese business book YouTube channel. Use a refined watercolor illustration style, pale cream background, teal and subtle gold accents, and a premium calm business-book-summary atmosphere. Leave the lower-left corner clean because the fixed Book Base logo will be composited after generation.

This is Scene 03: present the current book, the video's conclusion, and the supporting points in one finished image. Use the provided reference image as the actual current book cover from the start of image generation: {cover or 'current book cover reference unavailable'}. The reference cover must be naturally integrated into the whole composition as the main subject. Do not generate a different book, do not imitate only the title, and do not create a blank placeholder. Do not make the cover look pasted on later, floating, or like a separate layer.

{TEXT_LOCK_INSTRUCTION}:
{_text_block(elements)}

Composition:
- Left side: the real referenced book cover, large and naturally blended into the watercolor desk/background as the main subject.
- Right upper: label "今回の1冊".
- Right center: the video conclusion as the largest headline.
- Right middle to lower: 2-3 concise supporting point cards with small icons or mini headings.
- Right lower or background: a subtle person illustration, work scene, conversation scene, learning scene, consultation scene, growth motif, or other theme-linked business illustration.

Information design: one image must communicate the book, conclusion, supporting points, and who benefits. Add more useful information than Scenes 01-02, but keep text readable at YouTube reduced size. Avoid a mere book-cover introduction, decorative-only right side, clutter, English text, long script text, and any post-generation cover compositing."""
    return {"scene": 3, "scene_role": "今回の本紹介と動画全体の結論提示", "reference_image_required": True, "reference_image_path": cover, "reference_image_usage": "生成開始時点で実ブックカバーを参照画像として渡し、表紙に合わせて全体構図を生成する。後処理合成は禁止。", "core_message": conclusion, "supporting_summary": support, "target_viewer_label": target, "exact_text_elements": elements, "composition": "左側に実ブックカバー、右上に今回の1冊、右中央に動画の結論、右中段〜下段に補足要点2〜3個、右下または背景にテーマ連動の人物・仕事・会話イラスト", "visual_motifs": ["実ブックカバー", "動画の結論", "補足要点カード", "テーマ連動の人物・仕事・会話イラスト"], "style": _COMMON_STYLE_FOR_SCHEMA, "negative_rules": ["背景生成後に表紙を貼り付けない", "ブックカバーをAIに別物として再生成しない", "タイトルだけ似せた別の本にしない", "右側を意味の薄い飾りだけにしない", "長文を入れない"], "post_process": {"composite_real_book_cover": False, "cover_reference_at_generation": True}, "variation_key": "scene-03-reference-cover-integrated-conclusion-points", "final_prompt": final_prompt}
...
def _scene_04_structured_prompt(context: dict[str, object]) -> dict[str, object]:
    ...
    author_name = str(context.get("author_name", "著者"))
    scene_bodies = context.get("scene_bodies", {})
    scene4_body = str(scene_bodies.get("scene_04", "")) if isinstance(scene_bodies, dict) else ""
    credential_match = re.search(r"著者の.+?さんは、(.+?)人物です。", scene4_body)
    credential = _short_label(credential_match.group(1) if credential_match else "テーマに関わる知見を伝えてきた", "実践知を届ける著者", 18)
    sequence_label = "理解から実践へ" if re.search(r"実践|行動|明日", scene4_body) else "順番に学ぶ3ステップ"
    elements = [
        f"著者 {author_name}さん",
        credential,
        f"①{points[0]['label']}",
    ...
    ref = "Use the author reference only to create a refined watercolor portrait illustration that matches Book Base's world; do not paste the photo or make it photorealistic." if author_ref else "No author reference is available; do not imagine a face or a specific face. Use an elegant silhouette, back-view portrait, or book-and-pen motif to imply the author."
    final_prompt = f"""Create a 16:9 landscape video-insert image for Book Base, a Japanese business book YouTube channel. Use a refined watercolor illustration style with a pale cream background, teal and subtle gold accents, and an intelligent, gentle, premium atmosphere. Leave the lower-left corner clean because the fixed Book Base logo will be composited after generation. Do not draw or invent the Book Base logo.

This is Scene 04: show the author's credibility and organize the book's three valuable points as a learning roadmap. Do not use direct explanatory headings like "著者紹介" or "3つのポイント". The three points must not look like isolated labels; show that they should be understood in order and that the order leads to practical action.
...
Information design:
- Left side: large elegant author visual in watercolor, connected subtly to the right-side roadmap.
- Right upper: natural author heading and one short credibility line.
- Right middle: three ordered cards with ①, ②, ③. Each card should keep enough meaning to convey the book's value, not just a thin noun.
- Right lower or bottom: a small summary ribbon explaining the sequence, such as moving from understanding to practice.
- Use subtle icons only when they clarify each point. Keep text readable at YouTube reduced size.

Avoid: large headings "著者紹介" or "3つのポイント", word-only shallow point labels, merely stacking three unrelated cards, weak connection between author and points, clutter, long script text, English text, photorealistic portrait, and AI-generated logos."""
    return {"scene": 4, "scene_role": "著者の信頼性提示と重要ポイント3つの順序化", "reference_image_required": False, "reference_image_path": author_ref, "reference_image_usage": "著者参考画像がある場合のみBook Baseの世界観に合う水彩肖像に使う。なければ顔を想像せずシルエットや象徴表現にする。", "core_message": "著者の信頼性と順番に学ぶ3つの重要ポイント", "author_heading": elements[0], "author_credential_label": credential, "sequence_summary_label": sequence_label, "exact_text_elements": elements, "composition": SCENE_04_COMPOSITION, "visual_motifs": ["上品な著者水彩イラストまたはシルエット", "自然な著者見出し", "①②③の順序付きポイントカード", "ティールとゴールドの接続線", "順番の意味を示す補足帯"], "style": _COMMON_STYLE_FOR_SCHEMA, "negative_rules": ["「著者紹介」を大きく書かない", "「3つのポイント」を大きく書かない", "重要ポイントを単語だけにしない", "3項目をただ並べるだけにしない", "著者写真をそのまま貼らない", "AIにBook Baseロゴを描かせない", "長文を入れない"], "variation_key": "author-credibility-three-point-sequential-roadmap", "final_prompt": final_prompt}


def _scene_intro_theme_context(text: str) -> str:
    if re.search(r"会話|対話|伝え|聞き|雑談|言葉|コミュニケーション", text):
        return "communication: use conversation context, speech bubbles, facial expressions, and dialogue flow only when they match the current book"
    if re.search(r"管理|マネジメント|組織|チーム|会議|部下|上司|リーダ", text):
        return "management: use meeting, blueprint, flow, team structure, or work-system motifs"
    if re.search(r"お金|投資|資産|家計|収入|年収|貯", text):
        return "money/investing: use savings path, abstract chart, asset-building route, or balance motifs"
    if re.search(r"習慣|継続|行動|自己啓発|毎日|ルーティン", text):
        return "habit/self-development: use stairs, daily actions, continuity path, or branching road motifs"
    if re.search(r"思考|考え|判断|整理|ノート|問題解決", text):
        return "thinking method: use map, organized desk, notes, thought-flow, or framework motifs"
    if re.search(r"ストレス|心理|不安|安心|心|脳|感情", text):
        return "stress/psychology: use heart, brain, safety shield, and load-reduction motifs"
    if re.search(r"時間|予定|優先|締切|スケジュール", text):
        return "time management: use clock, calendar, priority flow, and schedule-sorting motifs"
    return "current-book theme: choose workplace, home, money, health, learning, or relationship context based only on the current script"


def _scene_intro_visual_metaphor(text: str, point_number: int) -> str:
    if re.search(r"迷|複雑|分から|混乱", text):
        return "maze with a clear route discovered"
    if re.search(r"つな|橋|移る|実践|行動", text):
        return "bridge from misunderstanding to practical action"
    if re.search(r"設計|構造|仕組み|組み立て|デザイン", text):
        return "blueprint or system diagram revealing the structure"
    if re.search(r"段階|ステップ|順番|積み上げ", text):
        return f"step {point_number} on an elegant staircase"
    if re.search(r"選|判断|分岐|優先", text):
        return "branching road with a decision compass"
    if re.search(r"比べ|対比|バランス|偏り", text):
        return "balanced scale contrasting common belief and book perspective"
    if re.search(r"守|安心|防|リスク", text):
        return "shield protecting the core lesson from common pitfalls"
    if re.search(r"地図|道筋|全体", text):
        return "map showing the route from common belief to book insight"
    return "flow diagram connecting common belief, hidden mechanism, consequence, and takeaway"


def _scene_intro_prompt(context: dict[str, object], scene: int, point_index: int) -> dict[str, object]:
    point = context["three_key_points"][point_index - 1]
    scene_bodies = context.get("scene_bodies", {})
    body = str(scene_bodies.get(f"scene_{scene:02d}", "")) if isinstance(scene_bodies, dict) else ""
    label = _short_label(re.sub(rf"^重要ポイントの{point_index}(?:つ目|つめ)は", "", body), str(point["label"]), 16)
    core = _short_label(body or str(point["core_message"]), str(point["label"]), 30)
    clauses = [c for c in re.split(r"。|、", body) if c]
    support = _short_label(next((c for c in clauses[1:] if c), core), "見直す視点", 18)
    practical = _short_label(clauses[-1] if clauses else core, "まず見直す", 16)
    common = _short_label(next((c for c in clauses if re.search(r"誤解|思い込み|ありがち|多く|つい|普通|一般", c)), "ありがちな見方"), "ありがちな見方", 12)
    book_view = _short_label(next((c for c in clauses if re.search(r"本書|構造|仕組み|本質|背景", c)), core), "本書の視点", 12)
    elements = [f"重要ポイント{'①②③'[point_index-1]}", label[:16], support[:18], f"誤解→{book_view[:8]}", practical[:16]]
    # Keep unique, readable elements while preserving the required point number.
    unique_elements = []
    for element in elements:
        if element and element not in unique_elements:
            unique_elements.append(element)
    elements = unique_elements[:3]
    theme_context = _scene_intro_theme_context(body)
    visual_metaphor = _scene_intro_visual_metaphor(body, point_index)
    contrast = f"{common} vs {book_view}"
    composition = "Top: key point number. Center: strong claim. Middle: information-rich diagram or scene illustration. Left/right: common belief versus book perspective. Bottom: summary ribbon with the practical takeaway. Leave lower-left clean for the composited Book Base logo."
    final_prompt = f"""Create a 16:9 landscape video-insert image for Book Base, a Japanese business book YouTube channel. Use a refined watercolor illustration style, pale cream background, teal and subtle gold accents, and a premium business-book-summary atmosphere. Leave the lower-left corner clean because the fixed Book Base logo will be composited after generation. Do not draw or invent the Book Base logo.

This is Scene {scene:02d}. Its fixed role is to introduce Key Point {point_index}, but it must not be a simple title card or chapter heading. Transform the current book's core insight into an information-rich introductory visual that viewers can understand intuitively. One image, one message, but include enough supporting structure to avoid becoming a title card. Do not create a generic business conversation image; avoid generic conversation scenes. Build every label, contrast, mechanism, scene context, and metaphor from the current script only; do not hard-code terms from any previous book. Do not create a generic checklist or planner image. For Scene 09, avoid repeating the Scene 08 CTA composition or Scene 05 Key Point 1 composition. For Scene 13, avoid repeating the Scene 09 or Scene 12 composition.

Main message:
{label}

Current Key Point {point_index}:
{label}

Core explanation from current script:
{core}

Contrast to visualize:
{contrast}

Mechanism visual idea:
Show how the common belief leads into the hidden structure and consequence, then point toward the practical takeaway.
...
Required elements: key point number, large claim, short supporting copy, diagram or scene illustration with people/context/structure/flow/contrast as appropriate, and a bottom summary ribbon. The image should make the point feel worth watching before the detailed explanation begins.

Avoid: a headline-only title card, abstract background only, generic pretty image, fixed topic words from a previous book, same composition every time, English text, clutter, long script text, and AI-generated logos."""
    return {"scene": scene, "fixed_role": f"重要ポイント{point_index}の導入", "scene_role": f"重要ポイント{'①②③'[point_index-1]}の導入", f"point_{point_index}_label": label, "core_message": core, f"point_{point_index}_core_message": core, "main_message": label, "contrast": contrast, "mechanism_visual": "common belief → hidden mechanism → consequence → practical takeaway", "practical_message": practical, "scene_context": theme_context, "visual_metaphor": visual_metaphor, "exact_text_elements": elements, "composition": composition, "visual_motifs": ["重要ポイント番号", "大きな主張", "誤解と本書の視点の対比", "構造図解", "下部まとめ帯"], "style": _COMMON_STYLE_FOR_SCHEMA, "negative_rules": ["見出しだけにしない", "抽象背景だけにしない", "特定書籍の語句を固定しない", "同じ構図を使い回さない", "AIにBook Baseロゴを描かせない"], "variation_key": f"scene-{scene:02d}-key-point-{point_index}-information-rich-intro", "final_prompt": final_prompt}


def _scene_05_structured_prompt(context: dict[str, object]) -> dict[str, object]:
    return _scene_intro_prompt(context, 5, 1)

def _topic_visual_scene(body: str, fallback: str) -> str:
    if re.search(r"会話|伝え|対話|相談|人間関係|コミュニケーション|言葉", body):
        return "conversation or consultation scene with speech bubbles and facial expression changes"
    if re.search(r"会議|チーム|上司|部下|組織|マネジメント|仕事|プロジェクト", body):
        return "workplace meeting, team workflow, decision board, or project map"
    if re.search(r"お金|投資|資産|家計|リスク|リターン|収入", body):
        return "asset-building path, simple chart, savings/investment cards, and risk-return balance"
    if re.search(r"習慣|継続|行動|成長|目標|毎日", body):
        return "daily habit loop, staircase, calendar, and small growth actions"
    if re.search(r"心理|感情|ストレス|安心|不安|回復|脳|心", body):
        return "mind and stress-load diagram with recovery flow and calm person scene"
    if re.search(r"時間|予定|優先|タスク|締切", body):
        return "clock, schedule board, priority axis, and task-flow cards"
    if re.search(r"思考|判断|整理|問題|仮説|問い|視点", body):
        return "map, notebook, branching choices, and structured thinking framework"
    return fallback

def _deep_dive_labels(body: str, point_label: str) -> dict[str, str]:
    clauses = [c.strip() for c in re.split(r"。|、", body) if c.strip()]
    def pick(pattern: str, fallback: str, limit: int = 14) -> str:
        return _short_label(next((c for c in clauses if re.search(pattern, c)), body or point_label), fallback, limit)
    main = _short_label(body or point_label, point_label, 18)
    misconception = pick(r"誤解|多く|ありがち|つい|だけ|一般|普通|失敗|勘違", "よくある誤解")
    correct = pick(r"本書|大事|重要|正しく|実際|ではなく|分け|視点|考え", "本書の見方")
    framework = pick(r"手順|判断|基準|分け|整理|まず|次に|型|フレーム|軸", "実践の判断軸")
    takeaway = pick(r"だから|つまり|一文|残|学び|防ぎ|変わ|つなが", main, 16)
    return {"main_concept": main, "misconception": misconception, "correct_view": correct, "framework": framework, "takeaway": takeaway}

def _deep_dive_visual_structure(body: str) -> str:
    if re.search(r"前|後|変化|改善|変わ|Before|After", body, flags=re.I):
        return "before_after"
    if re.search(r"手順|ステップ|順番|まず|次に|最後|流れ|プロセス", body):
        return "flow"
    if re.search(r"分け|区別|違い|要素|構造|分解|整理", body):
        return "decomposition"
    if re.search(r"誤解|悪い|良い|普通|従来|一方|ではなく|対比", body):
        return "contrast"
    return "metaphor"

def _deep_dive_composition(visual_structure: str) -> str:
    mapping = {
        "before_after": "Before / After layout comparing the common mistaken state with the improved book-based view; include a person or concrete scene tied to the current book theme, concise labels, and a bottom takeaway ribbon.",
        "flow": "Flow diagram layout showing input → mechanism/process → result or cause → practice → outcome; use short cards, arrows, a theme-matched person/setting illustration, and a bottom takeaway ribbon.",
        "decomposition": "Decomposition diagram layout separating concepts that are often confused; use 2-4 labeled parts, a concrete person/scene illustration, and a bottom takeaway ribbon.",
        "contrast": "Contrast layout comparing bad/common view and good/book view; avoid a generic split screen by using cards, balance scale, or layered notes plus a concrete scene and a bottom takeaway ribbon.",
        "metaphor": "Metaphor layout using a maze, staircase, bridge, map, blueprint, balance, shield, or compass that matches the extracted concept; include a theme-specific person/scene, concise diagram labels, and a bottom takeaway ribbon.",
        "step_demo": "Use a 3-step flow with concise cards, a concrete person/scene illustration, and a bottom takeaway ribbon.",
        "framework_demo": "Show an elegant worksheet or decision framework with concise labels, a concrete person/scene illustration, and a bottom takeaway ribbon.",
        "comparison": "Compare the mistaken view and book-based view with concise cards, a concrete person/scene illustration, and a bottom takeaway ribbon.",
        "action_map": "Show a practical action map or route with concise checkpoints, a concrete person/scene illustration, and a bottom takeaway ribbon.",
        "framework_application": "Show a framework being applied to a concrete target with concise cards, a person/scene illustration, and a bottom takeaway ribbon.",
        "scenario_demo": "Show a theme-matched scenario that contrasts common and book-based views, plus a bottom takeaway ribbon.",
        "practical_example": "Show a concrete use case with a person/scene illustration, short diagram labels, and a bottom takeaway ribbon.",
    }
    return mapping[visual_structure]
...
def _scene_06_structured_prompt(context: dict[str, object]) -> dict[str, object]:
    ...
    scene_bodies = context.get("scene_bodies", {})
    scene5_body = str(scene_bodies.get("scene_05", "")) if isinstance(scene_bodies, dict) else ""
    scene6_body = str(scene_bodies.get("scene_06", "")) if isinstance(scene_bodies, dict) else ""
    point_1_label = _short_label(re.sub(r"^重要ポイントの1つ目は", "", scene5_body), str(point["label"]), 18)
    labels = _deep_dive_labels(scene6_body or scene5_body, point_1_label)
    visual_structure = _deep_dive_visual_structure(scene6_body)
    composition = _deep_dive_composition(visual_structure)
    visual_metaphor = _topic_visual_scene(scene6_body + scene5_body, "conceptual bridge, map, or layered mechanism that explains Key Point 1 in depth")
    elements = [labels["main_concept"][:15], labels["correct_view"][:15], labels["takeaway"][:15]]
    final_prompt = f"""Create a 16:9 landscape deep-dive explanation image for Book Base, a Japanese business book YouTube channel. Use refined watercolor illustration, pale cream background, teal and subtle gold accents, and a premium business-book-summary atmosphere. Leave the lower-left corner clean because the fixed Book Base logo will be composited after generation. Do not draw a logo.

This is Scene 06: the deep-dive scene for Key Point 1 introduced in Scene 05. Do not create a generic business person image; avoid generic emotional icons. The image must not be a title card or abstract mood image. Convert the narration into short words, a diagram, and a concrete person/setting illustration so viewers understand the book's core concept.
...
- scene_number: 6
- point_number: 重要ポイント①
- main_concept: {labels['main_concept']}
- misconception: {labels['misconception']}
- correct_view: {labels['correct_view']}
- framework_elements: {labels['framework']}
- visual_metaphor: {visual_metaphor}
- takeaway_label: {labels['takeaway']}
...
1. Show one large claim that communicates the book's perspective, not just the point heading.
2. Include one diagram element selected from Before/After, bad vs good, flow, decomposition, decision axis, or metaphor according to visual_structure.
3. Include a concrete theme-matched person/scene illustration: {visual_metaphor}.
4. Add short supplemental keywords that make the diagram easy to absorb.
5. Add a bottom summary ribbon with the takeaway label.

Composition:
{composition}

Avoid: headline-only image, abstract background only, too little information, unrelated illustration, fixed visuals from a specific book, same layout every time, long script text, English text, and AI-generated logos."""
    return {"scene": 6, "fixed_role": "重要ポイント①の理由・背景・仕組み説明", "scene_role": "重要ポイント①の理由・背景・仕組み説明", "point_1_label": point_1_label, "core_message": labels["main_concept"], "scene_06_core_message": labels["main_concept"], "reason_label": labels["misconception"], "mechanism_label": labels["correct_view"], "effect_label": labels["takeaway"], "main_concept": labels["main_concept"], "misconception": labels["misconception"], "correct_view": labels["correct_view"], "framework_elements": labels["framework"], "takeaway_label": labels["takeaway"], "visual_metaphor": visual_metaphor, "visual_structure": visual_structure, "exact_text_elements": elements, "composition": composition, "visual_motifs": ["大きな主張", "図解", "人物・場面", "短い補足テキスト", "下部まとめ帯"], "style": _COMMON_STYLE_FOR_SCHEMA, "negative_rules": ["見出しだけにしない", "抽象背景だけにしない", "特定書籍の語句を固定しない", "原稿と無関係なイラストにしない", "AIにBook Baseロゴを描かせない"], "variation_key": f"scene-06-deep-dive-{visual_structure}", "final_prompt": final_prompt}
...
def _scene_09_structured_prompt(context: dict[str, object]) -> dict[str, object]:
    ...
    result = _scene_intro_prompt(context, 9, 2)
    point_2_type = _scene_09_point_type(scene9_body)
    result.update({
        "fixed_role": "重要ポイント②の導入",
        "scene_role": "重要ポイント②の導入",
        "point_2_type": point_2_type,
        "composition": _scene_09_composition(point_2_type) + " ただし単なる第2ポイントカードではなく、左右に誤解と本書の視点、中央に構造図解、下部に覚えるべき一言を入れる。",
        "variation_key": f"scene-09-key-point-2-{point_2_type}-information-rich-intro",
    })
    result["final_prompt"] = str(result["final_prompt"]).replace(
        "Composition:\nTop: key point number. Center: strong claim. Middle: information-rich diagram or scene illustration. Left/right: common belief versus book perspective. Bottom: summary ribbon with the practical takeaway. Leave lower-left clean for the composited Book Base logo.",
        "Point 2 type:\n" + point_2_type + "\n\nComposition:\n" + str(result["composition"]),
    ).replace(
        "Scene 09. Its fixed role is to introduce Key Point 2",
        "Scene 09. Its fixed role is to introduce Key Point 2 as an information-rich bridge from Key Point 1 to the next insight",
    )
    return result
...
def _scene_10_structured_prompt(context: dict[str, object]) -> dict[str, object]:
    ...
    point_2_label = _short_label(re.sub(r"^重要ポイントの2つ目は", "", scene9_body), str(point["label"]), 18)
    labels = _deep_dive_labels(scene10_body or scene9_body, point_2_label)
    visual_structure = {"flow": "step_demo", "decomposition": "framework_demo", "contrast": "comparison", "metaphor": "action_map"}.get(_deep_dive_visual_structure(scene10_body), _deep_dive_visual_structure(scene10_body))
    composition = _deep_dive_composition(visual_structure)
    visual_metaphor = _topic_visual_scene(scene10_body + scene9_body, "practical map, process cards, or framework worksheet that explains Key Point 2 in depth")
    elements = [labels["main_concept"][:15], labels["correct_view"][:15], labels["takeaway"][:15]]
    final_prompt = f"""Create a 16:9 landscape deep-dive explanation image for Book Base, a Japanese business book YouTube channel. Use refined watercolor illustration, pale cream background, teal and subtle gold accents, and a premium business-book-summary atmosphere. Leave the lower-left corner clean because the fixed Book Base logo will be composited after generation. Do not draw a logo.

This is Scene 10: the deep-dive scene for Key Point 2 introduced in Scene 09. The image must translate the narration into a useful explanatory visual, not a generic title card, generic flowchart, or unrelated business people. Do not create a generic cause-and-effect flowchart unless the current script actually requires it.
...
Required composition:
1. Show one large claim that communicates the book's perspective behind Key Point 2.
2. Include a diagram element: Before/After, bad vs good, flow, decomposition, decision axis, 3-step process, or metaphor according to the content.
3. Include a concrete theme-matched person/scene illustration: {visual_metaphor}.
4. Place concise supplemental keywords around the diagram so the idea is absorbable.
5. Add a bottom summary ribbon with the takeaway label.

Composition:
{composition}

Differentiate from Scene 09's introductory card and Scene 11's real-life episode. Avoid headline-only image, abstract background only, too little information, unrelated illustration, fixed visuals from a specific book, same layout every time, long script text, English text, AI-generated logos; avoid generic split-screen flowcharts; avoid generic business people; avoid repeating the Scene 09 composition."""
    return {"scene": 10, "fixed_role": "重要ポイント②の具体化", "scene_role": "重要ポイント②の深掘り", "core_message": labels["main_concept"], "point_2_label": point_2_label, "scene_10_core_message": labels["main_concept"], "example_label": labels["misconception"], "application_label": labels["framework"], "result_label": labels["takeaway"], "main_concept": labels["main_concept"], "misconception": labels["misconception"], "correct_view": labels["correct_view"], "framework_elements": labels["framework"], "takeaway_label": labels["takeaway"], "visual_structure": visual_structure, "visual_metaphor": visual_metaphor, "exact_text_elements": elements, "composition": composition, "visual_motifs": ["大きな主張", "図解", "人物・場面", "短い補足テキスト", "下部まとめ帯"], "style": _COMMON_STYLE_FOR_SCHEMA, "negative_rules": ["見出しだけにしない", "抽象背景だけにしない", "特定書籍の語句を固定しない", "scene_09やscene_11と混同しない", "AIにBook Baseロゴを描かせない"], "variation_key": f"scene-10-deep-dive-{visual_structure}", "final_prompt": final_prompt}
...
def _scene_13_structured_prompt(context: dict[str, object]) -> dict[str, object]:
    ...
    result = _scene_intro_prompt(context, 13, 3)
    point_3_type = _scene_13_point_type(scene13_body)
    result.update({
        "fixed_role": "重要ポイント③の導入",
        "scene_role": "重要ポイント③の導入",
        "point_3_type": point_3_type,
        "composition": _scene_13_composition(point_3_type) + " ただし単なる第3ポイントカードではなく、左右に誤解と本書の視点、中央に構造図解、下部に覚えるべき一言を入れる。",
        "variation_key": f"scene-13-key-point-3-{point_3_type}-information-rich-intro",
    })
    result["final_prompt"] = str(result["final_prompt"]).replace(
        "Composition:\nTop: key point number. Center: strong claim. Middle: information-rich diagram or scene illustration. Left/right: common belief versus book perspective. Bottom: summary ribbon with the practical takeaway. Leave lower-left clean for the composited Book Base logo.",
        "Point 3 type:\n" + point_3_type + "\n\nComposition:\n" + str(result["composition"]),
    ).replace(
        "Scene 13. Its fixed role is to introduce Key Point 3",
        "Scene 13. Its fixed role is to introduce Key Point 3 as an information-rich final major-point introduction",
    )
    return result
...
def _scene_14_structured_prompt(context: dict[str, object]) -> dict[str, object]:
    ...
    point_3_label = _short_label(re.sub(r"^重要ポイントの3つ目は", "", scene13_body), str(point["label"]), 18)
    labels = _deep_dive_labels(scene14_body or scene13_body, point_3_label)
    visual_structure = {"flow": "step_demo", "decomposition": "framework_application", "contrast": "scenario_demo", "metaphor": "practical_example"}.get(_deep_dive_visual_structure(scene14_body), _deep_dive_visual_structure(scene14_body))
    composition = _deep_dive_composition(visual_structure)
    visual_metaphor = _topic_visual_scene(scene14_body + scene13_body, "final practical framework, bridge, compass, or checklist-in-use that explains Key Point 3 in depth")
    elements = [labels["main_concept"][:15], labels["correct_view"][:15], labels["takeaway"][:15]]
    final_prompt = f"""Create a 16:9 landscape deep-dive explanation image for Book Base, a Japanese business book YouTube channel. Use refined watercolor illustration, pale cream background, teal and subtle gold accents, and a premium business-book-summary atmosphere. Leave the lower-left corner clean because the fixed Book Base logo will be composited after generation. Do not draw a logo.

This is Scene 14: the deep-dive scene for Key Point 3 introduced in Scene 13. The image must make the final major point understandable as a structure, judgment axis, or practical framework, not a generic meeting scene or a mere title card. Do not create a generic meeting scene unless the current script actually requires it.
...
Required composition:
1. Show one large claim that communicates the book's perspective behind Key Point 3.
2. Include a diagram element: Before/After, bad vs good, flow, decomposition, decision axis, 3-step process, or metaphor according to the content.
3. Include a concrete theme-matched person/scene illustration: {visual_metaphor}.
4. Place concise supplemental keywords around the diagram so the idea is absorbable.
5. Add a bottom summary ribbon with the takeaway label.
...
Differentiate from Scene 13's introductory card, Scene 15's quote reinforcement, and Scene 18's whole-video implementation scene. Avoid headline-only image, abstract background only, too little information, unrelated illustration, fixed visuals from a specific book, same layout every time, long script text, English text, AI-generated logos, and avoid repeating the Scene 13 composition or the later Scene 18 implementation composition."""
    return {"scene": 14, "fixed_role": "重要ポイント③の具体化", "scene_role": "重要ポイント③の深掘り", "point_3_label": point_3_label, "core_message": labels["main_concept"], "scene_14_core_message": labels["main_concept"], "example_context_label": labels["misconception"], "action_label": labels["framework"], "result_label": labels["takeaway"], "main_concept": labels["main_concept"], "misconception": labels["misconception"], "correct_view": labels["correct_view"], "framework_elements": labels["framework"], "takeaway_label": labels["takeaway"], "visual_structure": visual_structure, "visual_metaphor": visual_metaphor, "exact_text_elements": elements, "composition": composition, "visual_motifs": ["大きな主張", "図解", "人物・場面", "短い補足テキスト", "下部まとめ帯"], "style": _COMMON_STYLE_FOR_SCHEMA, "negative_rules": ["見出しだけにしない", "抽象背景だけにしない", "特定書籍の語句を固定しない", "scene_13やscene_18と混同しない", "AIにBook Baseロゴを描かせない"], "variation_key": f"scene-14-deep-dive-{visual_structure}", "final_prompt": final_prompt}
...
SCENE_03_FORBIDDEN_META_PHRASES = (
    "【シーン3】のままにしてください",
    "省略しないでください",
    "改行は本文内に入れないでください",
    "【シーン3】のままにします",
...
def _remove_scene3_meta_instructions(text: str) -> str:
    cleaned = " ".join(text.splitlines())
    for phrase in SCENE_03_FORBIDDEN_META_PHRASES:
        cleaned = cleaned.replace(phrase, "")
    cleaned = re.sub(r"\s+", " ", cleaned).strip()
    return cleaned


def _extract_author_name_from_source(source_text: str) -> str:
    patterns = [
        r"(?:著者|作者|著)[:：\s]*([^\n、。『』「」:：\s]{2,30})",
        r"([^\n、。『』「」]{2,30})さんの『",
        r"『[^』]+』(?:の)?(?:著者|作者)[は、\s]*([^\n、。『』「」]{2,30})",
...
def _scene3_conclusion_from_body(body: str, source_text: str) -> str:
    body = _remove_scene3_meta_instructions(body)
    patterns = [r"結論から言うと、(.+?)です。", r"結論は、(.+?)です。", r"中心メッセージ[:：\s]*(.+?)(?:。|\n)"]
    for pattern in patterns:
        match = re.search(pattern, body) or re.search(pattern, source_text)
        if match:
            return match.group(1).strip("。です ")[:28]
    return _short_label(source_text, "仕事や人間関係を少し楽にする視点が学べる", 32)


def _scene3_target_label(body: str, source_text: str) -> str:
    combined = f"{body} {source_text}"
    if re.search(r"会話|伝え|言い換|人間関係|コミュニケーション|相談", combined):
        return "相手との関係を壊さずに伝えたい人"
    if re.search(r"お金|資産|投資|家計|年収|副業", combined):
        return "お金の判断力を高めたい人"
    if re.search(r"習慣|継続|行動|実践", combined):
        return "学びを行動に変えたい人"
    return "仕事や日常に本の学びを活かしたい人"


def _extract_author_credential(source_text: str, existing_body: str = "") -> str:
    combined = f"{source_text}\n{_remove_scene4_meta_instructions(existing_body)}"
    patterns = [
        r"(?:プロフィール|経歴|肩書き)[:：\s]*([^。\n]{8,80})",
        r"(?:著者|作者|著)[^\n。]{0,20}(?:は|:|：)\s*([^。\n]{12,70}(?:してきた|携わってきた|支えてきた|伝えてきた|研究してきた|発信してきた|実践してきた)[^。\n]{0,20})",
    ]
    for pattern in patterns:
        match = re.search(pattern, combined)
        if match:
            credential = re.sub(r"^[、。\s]+", "", match.group(1))
            return _short_label(credential, "本書のテーマに関わる知見を積み重ねてきた", 54)
    if re.search(r"会話|伝え|言い換|人間関係|コミュニケーション|対話|心理", combined):
        return "伝え方や人間関係の課題に向き合い、実践に使える知見を届けてきた"
    if re.search(r"お金|投資|資産|家計|経済|会計", combined):
        return "お金や意思決定の知識を、日常の判断に活かせる形で伝えてきた"
    if re.search(r"習慣|行動|実践|継続|目標", combined):
        return "学びを行動に変える方法を、実践しやすい形で伝えてきた"
    return "本書のテーマに関わる知見を、読者が使いやすい形で伝えてきた"


def _extract_value_point(seed: str, fallback: str) -> str:
    seed = _remove_scene4_meta_instructions(seed)
    seed = re.sub(r"^重要ポイントの[123一二三１２３]つ目は", "", seed)
    first_sentence = re.split(r"。", seed)[0].strip(" 、。")
    if 10 <= len(first_sentence) <= 34:
        return first_sentence
    label = _short_label(seed, fallback, 26)
    return label if len(label) >= 8 else fallback


def build_scene4_script_body(*, source_text: str, existing_body: str, scenes: Any) -> str:
    author = _extract_author_name_from_source(source_text)
    credential = _extract_author_credential(source_text, existing_body)
    if not re.search(r"(してきた|携わってきた|支えてきた|伝えてきた|研究してきた|発信してきた|実践してきた)$", credential):
        credential = credential.rstrip("。") + "として活動してきた"
    scene_map: dict[int, str] = {}
    if isinstance(scenes, list):
        for item in scenes:
            if isinstance(item, dict):
                number = item.get("scene_number") or item.get("scene") or item.get("シーン番号")
                body = str(item.get("body") or item.get("text") or item.get("content") or "")
                if isinstance(number, int):
                    scene_map[number] = body
    p1 = _extract_value_point(scene_map.get(5, existing_body), "最初に本質的な課題を見える形にすること")
    p2 = _extract_value_point(scene_map.get(9, existing_body), "次に背景や仕組みを理解して判断を整えること")
    p3 = _extract_value_point(scene_map.get(13, existing_body), "最後に小さな行動へ落として実践を始めること")
    body = (
        f"著者の{author}さんは、{credential}人物です。"
        f"本書の重要ポイントは、{p1.rstrip('です')}、{p2.rstrip('です')}、{p3.rstrip('です')}です。"
        "この三つは横並びの知識ではなく、課題をつかみ、背景を深掘りし、行動へ移すための順番です。"
        "仕事や日常で学びをどう使えばいいかで悩む方は、この三つをつなげると、明日からの実践に迷いなく自然に入りやすいです。"
    )
    return _remove_scene4_meta_instructions(body)


def build_scene3_script_body(*, source_text: str, book_title: str, existing_body: str) -> str:
    author = _extract_author_name_from_source(source_text)
    conclusion = _scene3_conclusion_from_body(existing_body, source_text)
    target = _scene3_target_label(existing_body, source_text)
    support = "本書が教えてくれるのは、気合いで乗り切る方法ではなく、状況の見方を変える技術です。仕事の会議や上司への相談でも、見方が変わるだけで言葉の選び方は変わります。だからこそ、この本の学びは知識で終わらず、明日の判断にそのままつながります。"
    body = f"今回紹介するのは、{author}さんの『{book_title}』こちらの本になります。結論から言うと、{conclusion}です。{support}{target}にとって、とても学びのある一冊になっています。"
    return _remove_scene3_meta_instructions(body)


def _apply_scene3_and_scene4_script_templates(scenes: Any, *, source_text: str, book_title: str) -> Any:
    if not isinstance(scenes, list):
        return scenes
    for item in scenes:
        if isinstance(item, dict) and (item.get("scene_number") or item.get("scene") or item.get("シーン番号")) == 3:
            existing = str(item.get("body") or item.get("text") or item.get("content") or "")
            item["body"] = build_scene3_script_body(source_text=source_text, book_title=book_title, existing_body=existing)
            item.pop("text", None)
            item.pop("content", None)
            break
    for item in scenes:
        if isinstance(item, dict) and (item.get("scene_number") or item.get("scene") or item.get("シーン番号")) == 4:
            existing = str(item.get("body") or item.get("text") or item.get("content") or "")
            item["body"] = build_scene4_script_body(source_text=source_text, existing_body=existing, scenes=scenes)
            item.pop("text", None)
            item.pop("content", None)
            break
    return scenes
...
def validate_bookbase_script(script: str) -> list[str]:
    ...
    for index, match in enumerate(matches):
        ...
        if number == 3:
            if any(phrase in body for phrase in SCENE_03_FORBIDDEN_META_PHRASES):
                errors.append("シーン3：メタ指示文が混入しています")
            if not body.startswith("今回紹介するのは、"):
                errors.append("シーン3：冒頭固定文で始まっていません")
            if not re.search(r"^今回紹介するのは、.+?さんの『.+?』こちらの本になります。結論から言うと、.+?です。", body):
                errors.append("シーン3：本紹介と結論提示の固定文型ではありません")
            if not body.endswith("にとって、とても学びのある一冊になっています。"):
                errors.append("シーン3：締め固定文で終わっていません")
        if number == 4:
            if any(phrase in body for phrase in SCENE_04_FORBIDDEN_META_PHRASES):
                errors.append("シーン4：メタ指示文が混入しています")
            if not re.search(r"^著者の.+?さんは、.+?人物です。", body):
                errors.append("シーン4：冒頭固定文型ではありません")
            if not re.search(r"本書の重要ポイントは、.+?、.+?、.+?です。", body):
                errors.append("シーン4：重要ポイント3点の固定文型ではありません")
            if not re.search(r"順番|この三つ|つなげる|実践|明日", body):
                errors.append("シーン4：3点を順番に学ぶ意味と実践接続が不足しています")
        if number in {6, 10, 14} and len(set(body)) >= 5:
            if re.search(r"省略|改行|プロンプト|指示文|メタ文|のままに", body):
                errors.append(f"シーン{number}：メタ指示文が混入しています")
            if not re.search(r"本書|ここで|一流|大事|問題|重要", body):
                errors.append(f"シーン{number}：本書の問題提起やテーマ提示が不足しています")
            if not re.search(r"多く|ありがち|誤解|つい|一般|失敗|勘違", body):
                errors.append(f"シーン{number}：誤解・ありがちな状態の説明が不足しています")
            if not re.search(r"仕組み|構造|背景|なぜ|原因|分け|区別|判断軸|手順|フレーム", body):
                errors.append(f"シーン{number}：中核概念の構造分解が不足しています")
            if not re.search(r"明日|実践|見直|行動|使|手順|判断|整理", body):
                errors.append(f"シーン{number}：実践に落とす説明が不足しています")
...
def _build_image_prompt_item(scene: int, context: dict[str, object] | None = None, asset_check: AssetCheck | None = None) -> dict[str, object]:
    ...
    if scene_01_prompt:
        ...
        asset_note = "実ブックカバーを参照画像として生成開始時点で渡し、表紙に合わせて全体構図を生成します。後処理で表紙合成しません。" if used_image != "なし" else "scene_03：NEEDS_REVIEW。理由：今回の本のブックカバーが見つかりません。架空の表紙は作らないでください。"
...
def generate_fallback_assets(source_text: str, book_name: str, asset_checks: list[AssetCheck] | None = None) -> GeneratedAssets:
    ...
    scene_leads = {
        ...
        3: build_scene3_script_body(source_text=source_text, book_title=book_title, existing_body="結論から言うと、本の学びを明日の仕事で使える形に整えることです。"),
        4: build_scene4_script_body(source_text=source_text, existing_body="", scenes=[{"scene_number": 5, "body": "重要ポイントの1つ目は問題を見える化することです。"}, {"scene_number": 9, "body": "重要ポイントの2つ目は原因や背景を捉えることです。"}, {"scene_number": 13, "body": "重要ポイントの3つ目は小さく実践へ移すことです。"}]),
...
def generate_ai_assets(
    ...
    *,
    ...
    client = OpenAI(api_key=os.environ.get("OPENAI_API_KEY"), timeout=180.0, max_retries=5)
...
scene_05 / scene_09 / scene_13は、重要ポイントを初めて紹介する導入シーンとして深く説明してください。各シーン生成前にcore_claim（核心主張）、common_belief（ありがちな誤解・思い込み）、mechanism（問題が起きる構造）、consequence（放置した場合の悪影響）、practical_takeaway（視聴者が見直すこと）を入力本文から抽出し、「主張 → 誤解 → 構造 → 悪影響 → 学ぶ意味」の流れで原稿化してください。scene_05は必ず「重要ポイントの1つ目は、〜、ということです。」、scene_09は必ず「重要ポイントの2つ目は、〜、ということです。」、scene_13は必ず「重要ポイントの3つ目は、〜、ということです。」で始めてください。章タイトルの羅列、一般論だけの説明、「大切です」「重要です」「意識しましょう」だけの浅い締め、過去テーマ語句や今回の本だけの語句のロジック固定、プロンプト指示文混入は禁止です。文章は短く切りすぎず、誤解・構造・悪影響・次の詳細解説を聞く理由を自然につなげてください。
...
scene_number=3のbodyは本紹介と動画冒頭の結論提示です。必ず「今回紹介するのは、著者名さんの『書名』こちらの本になります。結論から言うと、結論です。」で始めてください。著者名と書名は入力本文の基準データから正確に使い、書名は上記の書籍名を使ってください。「動画の結論は〜」は禁止し、「結論から言うと、〜です。」に統一してください。中盤は本の内容を一言で終わらせず、結論を支える考え方、本書が教える技術・視点・考え方、仕事や人間関係でどう役立つかを2〜3文で補足し、視聴者が自分事化できるコメントを入れてください。末尾は必ず「〜したい人にとって、とても学びのある一冊になっています。」で締めてください。「【シーン3】のままにしてください」「省略しないでください」「改行は本文内に入れないでください」「【シーン3】のままにします」「省略不可です」などのメタ指示文は絶対に混入させないでください。
scene_number=4のbodyはシーン4専用テンプレートに従い、「著者の〜さんは、〜してきた人物です。」で始めてください。著者名と経歴は入力本文の基準データから取得し、信頼性やテーマとの関連が伝わる経歴を優先して要約してください。続けて必ず「本書の重要ポイントは、〜、〜、〜です。」の形で、本の良さを抽出した3点を短文レベルで整理してください。章タイトルや薄い単語の羅列は禁止です。最後は、3点が横並びではなくこの順番で理解する意味、知識を深掘りして明日からの実践に迷いなく入れることを自然に説明し、「〜で悩む方は、この三つをつなげると、〜です。」の方向で締めてください。本文内改行、箇条書き、「【シーン4】ずっと【シーン4】のままにしてください」「省略しないでください」「改行は本文内に入れないでください」などのメタ指示文は絶対に混入させないでください。
scene_number=6/10/14のbodyは、それぞれ直前の重要ポイントを深掘りする本論シーンです。scene_number=6はscene_number=5の重要ポイント①、scene_number=10はscene_number=9の重要ポイント②、scene_number=14はscene_number=13の重要ポイント③を一段深く説明してください。各シーンの生成前に対象書籍の内容から key_concept, problem_definition, common_misunderstanding, mechanism, distinction, practical_framework, consequence, takeaway を内部抽出し、本文には「問題提起 → 誤解・ありがちな状態 → 中核概念の構造分解 → 実践フレーム → 学びの一文」の流れを必ず入れてください。見出しの繰り返し、一般論、短い具体例だけの説明は禁止です。本書が問題視するテーマ、なぜその問題が起きるか、分けて考える概念、判断軸・手順・構造、理解しない場合の問題、最後に残す一文まで含め、規定文字数内で情報密度を高くしてください。始め方は固定せず、本の内容に合わせて自然にし、会話調だが軽くなりすぎないナレーションにしてください。本文内改行、箇条書き、プロンプト指示文、メタ文、特定書籍や過去テーマに固定した語句は禁止です。
...
画像生成前提として、内部で image_context.json 相当の book_title, author_name, current_theme, quiz, three_key_points, assets を原稿と入力画像チェックから抽出してください。scene_01〜scene_05は「固定役割」と「可変内容」を必ず分離し、具体的な正解文言、テーマ語句、ポイント語句、タイトル、著者名、book_cover/authorパスは毎回の原稿とinputから生成してください。scene_02の固定役割は「クイズ正解発表と意味づけ・テーマへの橋渡し」です。scene_02画像は、画面の約50%を正解発表エリア、残り約50%を原稿後半と連動した補足情報エリアにしてください。補足情報エリアには、世間のイメージとのズレ、本書が示す本質、この後に学ぶテーマを短く表示してください。左側または中央に「正解：〜」を大きく置き、反対側に「一般的な見方 → 本書の視点」の対比情報を置いてください。正解だけ、A/B/Cだけ、意味づけと無関係なイラスト、文字が少なすぎる画像は禁止です。Book Baseロゴは固定ロゴを後処理で合成するため、AIにロゴを描かせないでください。scene_03画像は、ブックカバーを後から埋め込む方式を禁止します。生成開始時点で実際のブックカバー画像を参照画像として渡し、その表紙に合わせて全体構図を作ってください。左側に実ブックカバー、右上に「今回の1冊」、右中央に動画の結論、右中段〜下段に補足要点2〜3個、右下または背景にテーマ連動の人物・仕事・会話・学習イラストを置き、1枚で今回の本・結論・補足要点・誰に役立つかが伝わる情報設計にしてください。後から貼った感じ、浮いた感じ、別レイヤー感、表紙と別の本オブジェクト生成、タイトルだけ似せた別物、右側が飾りだけの構図は禁止です。scene_04画像はシーン4専用構図として、左側に著者の大きな水彩イラストまたは上品なシルエット、右上に「著者 〜さん」と短い肩書き、右中に①②③の重要ポイント、右下または下部に順番に学ぶ意味を示す補足帯を必ず入れてください。「著者紹介」「3つのポイント」という直球見出しを大きく書くことは禁止です。3点は単語だけにせず、本の価値が伝わる短い要約にし、細い矢印や線で①→②→③の順序が実践へつながることを見せてください。16:9、水彩画風、淡いクリーム背景、ティール・ゴールドアクセント、高級感・知性・やさしさを保ち、Book Baseロゴは後処理合成のためAIに描かせないでください。scene_06の固定役割は「重要ポイント①の深掘り」です。scene_06ではscene_05の重要ポイント①とscene_06本文から、scene_number, point_number, main_concept, misconception, correct_view, framework_elements, process_flow, before_after, visual_metaphor, takeaway_labelを毎回抽出してください。画像は大きな主張、Before/After・悪い例/良い例・フロー図・分解図・判断軸・因果関係・3ステップ・メタファーのいずれか、テーマに合った人物・場面イラスト、短い補足テキスト、下部のまとめ帯を必ず含めてください。単なる見出し画像、抽象背景だけ、情報量が少ない画像、原稿と関係が薄いイラスト、特定の本や過去テーマの固定は禁止です。各sceneのexact_text_elementsを必ず原稿由来で作り、最終プロンプトには必ず次の英文を入れてください: Use only the following Japanese text elements exactly as written. Do not add any other Japanese or English text.
scene_05 / scene_09 / scene_13画像は、単なる重要ポイントの見出し画像にせず、重要ポイントの中身を視聴者が直感的に理解できる情報量のある導入ビジュアルにしてください。各シーンでpoint_number, main_message, contrast, mechanism_visual, practical_message, scene_context, visual_metaphorを現在の原稿と本のテーマから抽出し、重要ポイント番号、大きな主張、補足コピー、図解またはシーンイラスト、下部のまとめ帯を必ず含めてください。上部：番号、中央：主張、中段：図解・人物・構造、左右：誤解と本書の視点の対比、下部：まとめ帯を基本に、コミュニケーション・マネジメント・お金・習慣・思考法・心理・時間術などテーマに合わせて場面と比喩を変えてください。16:9、水彩画風、淡いクリーム背景、ティール・ゴールドアクセント、高級感を守り、Book Baseロゴは後処理合成のためAIに描かせないでください。見出しだけ、抽象背景だけ、ただ綺麗なだけ、毎回同じ構図、過去テーマ語句や特定書籍ビジュアルの固定は禁止です。
scene_09の固定役割は「重要ポイント②の導入」です。scene_09ではscene_09本文からpoint_2_label, point_2_core_message, point_2_type(method / mindset / habit / process / contrast / skill / framework), visual_metaphorを毎回抽出してください。会話、言い換え、受け取り方、視点変更、2人のビジネスパーソン、コミュニケーション、相手に伝える等を恒久テンプレートに固定しないでください。scene_09はscene_05およびscene_08との差別化を明記してください。
scene_10の固定役割は「重要ポイント②の深掘り」です。scene_10ではscene_09の重要ポイント②とscene_10本文から、scene_number, point_number, main_concept, misconception, correct_view, framework_elements, process_flow, before_after, visual_metaphor, takeaway_labelを毎回抽出し、深掘り原稿を短い言葉・図解・人物イラスト・下部まとめ帯へ変換してください。exact_text_elementsは3つ以内、原稿由来の短い日本語のみとし、英語や長文を入れないでください。split-screen、cause and effect、flowchart、business people in conversation、コミュニケーション、言い換え、視点変更、原因と結果を恒久テンプレートに固定しないでください。scene_10はscene_09の導入カードを繰り返さず、scene_11の実話エピソード画像とも役割を分けてください。
...
scene_14の固定役割は「重要ポイント③の深掘り」です。scene_14ではscene_13の重要ポイント③とscene_14本文から、scene_number, point_number, main_concept, misconception, correct_view, framework_elements, process_flow, before_after, visual_metaphor, takeaway_labelを毎回抽出し、深掘り原稿を短い言葉・図解・人物イラスト・下部まとめ帯へ変換してください。exact_text_elementsは3つ以内、原稿由来の短い日本語のみとし、英語や長文を入れないでください。meeting、businessperson calmly responding、会議、返答、上司への対応、コミュニケーション、言い換え、具体的フレーズを恒久テンプレートに固定しないでください。scene_14はscene_13の導入カードを繰り返さず、scene_18の全体実践シーンとも役割を分けてください。
...
""".strip()
    ...
    for attempt in range(1, 4):
        ...
        try:
            data["scenes"] = _apply_scene3_and_scene4_script_templates(data["scenes"], source_text=source_text, book_title=book_name)
            ...
            return GeneratedAssets(
                ...
                api_communication_report=format_openai_api_report(api_records),
            )
