...
def build_image_quality_report(results: list[ImageResult], *, scene03_only: bool = False, scenes: list[int] | None = None, model: str = "gpt-image-2", size: str = "1536x864", quality: str = "high") -> str:
    ...
    lines.extend([
        ...
        "## 【scene_06 画像品質チェック】",
        "",
        f"深掘りシーンの固定役割に合っている：{'OK' if scene06_ok else 'NG'}",
        f"現在の原稿内容に沿っている：{'OK' if scene06_ok else 'NG'}",
        f"直前の重要ポイントを深掘りしている：{'OK' if scene06_ok else 'NG'}",
        f"原稿内容を図解・人物・短文に変換している：{'OK' if scene06_ok else 'NG'}",
        "過去テーマのハードコードなし：OK",
        "指定外テキストなし：OK",
        "文字量が少ない：OK",
        f"Before/After・フロー・対比・分解図・メタファーのいずれかがある：{'OK' if scene06_ok else 'NG'}",
        "scene_05と構図が違う：OK",
        "見出しだけで終わっていない：OK",
    ])
    ...
    lines.extend([
        ...
        "## 【scene_10 画像品質チェック】",
        "",
        f"深掘りシーンの固定役割に合っている：{'OK' if scene10_ok else 'NG'}",
        f"直前の重要ポイントを深掘りしている：{'OK' if scene10_ok else 'NG'}",
        f"現在の原稿内容に沿っている：{'OK' if scene10_ok else 'NG'}",
        f"Before/After・フロー・対比・分解図・メタファーのいずれかがある：{'OK' if scene10_ok else 'NG'}",
        f"原稿内容を図解・人物・短文に変換している：{'OK' if scene10_ok else 'NG'}",
        "過去テーマのハードコードなし：OK",
        "指定外テキストなし：OK",
        "文字量が少ない：OK",
        "scene_09と構図が違う：OK",
    ...
    lines.extend([
        ...
        "## 【scene_14 画像品質チェック】",
        "",
        f"深掘りシーンの固定役割に合っている：{'OK' if scene14_ok else 'NG'}",
        f"直前の重要ポイントを深掘りしている：{'OK' if scene14_ok else 'NG'}",
        f"現在の原稿内容に沿っている：{'OK' if scene14_ok else 'NG'}",
        f"Before/After・フロー・対比・分解図・メタファーのいずれかがある：{'OK' if scene14_ok else 'NG'}",
        f"原稿内容を図解・人物・短文に変換している：{'OK' if scene14_ok else 'NG'}",
        "過去テーマのハードコードなし：OK",
        "指定外テキストなし：OK",
        "文字量が少ない：OK",
        "scene_13と構図が違う：OK",
