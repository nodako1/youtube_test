...
- 原稿本文から画像用テキストを抽出するときは、固定挨拶、出典説明、テーマ接続文、固定締め文は除き、短いクイズ文と数値入りA/B/C選択肢だけを使う。
- 会社員が自分ごと化しやすいテーマにする。
...
1. 今回の原稿本文から、scene_01の統計クイズ文を1つ抽出する。固定挨拶、出典説明、テーマ接続文、固定締め文は画像内に入れない。
2. そのクイズ文に対して、％・人数・社数など具体的な数値単位を含む短いA/B/Cの選択肢を抽出する。
3. クイズは実在する会社・団体の調査や統計データ由来に見えるようにする。
4. 画像内の文字は最小限にする。
5. 答えは絶対に含めない。
